function [MMP, matfilename] = Process_McLane_WFP_Deployment(metadata_filename)
% radMMP version 2.11c (c = OOI Coastal) 
% RADesiderio, Oregon State University, 2020-05-08
%=========================================================================
% DESCRIPTION:
...
% USAGE:
%
%    [MMP] = Process_McLane_WFP_Deployment('metadata_WFP010.txt');
%
%         No data products are saved in a matfile.
%
%    [MMP, matfilename] = Process_McLane_WFP_Deployment('metadata_WFP010.txt');
%
...
%..             excised profiles not selected to be processed (nan-filled) from MMP fields
...
if nargin~=1
    disp('USAGE: MMP = Process_McLane_WFP_Deployment_Function(metadata_filename)');
...
for ii = prof2proc
    ...
    par(ii) = nan_bad_profile_sections(par(ii));
end
ctd_L1_NaNd = ctd;
flr_L1_NaNd = flr;
par_L1_NaNd = par;
...
%.. par and flr binning
[pr_min, binsize, pr_max] = ...
    determine_binning_parameters(par, 'pressure');
disp('Begin pressure binning par profile data');
for ii = prof2proc
    par(ii) = pressure_bin_mmp_data(par(ii), pr_min, binsize, pr_max);
end
par_L2 = par;
...
MP_par_Nand = write_field_arrays_to_new_structure(par_L1_NaNd, 'nan_processed_par_', prof2proc);
...
MP_binned_par.L1_Section = 'L1: PROCESSED, NOT BINNED (use [fast]scatter.m)';
MP_par_Nand.L0_Section = 'L0: RAW DATA (use [fast]scatter.m)'; 

MMP = amalgamate_scalar_structures( {MP_aux, ...
