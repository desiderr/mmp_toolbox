...
%              MPacm_L0, a scalar structure of NaN-padded ragged 2D arrays of L2 data.           
%
%
...
if contains(mode, 'import')
    ...
    for ii = meta.profiles_to_process
        ...
        aqd(ii) = import_A_ad2cp(Afilename); %#ok<*AGROW,*SAGROW>
    end
    disp('... import done.');
    toc
...
aqd = void_short_profiles(aqd, 'heading', meta.acm_nptsMin, -1);
aqd = aqd_set_processing_fields(aqd, meta);
disp('... deal done');
...
acm_L1 = aqd_set_sensor_field_indices(aqd, 'L1');
for ii = prof2proc; acm_L1_Nand(ii) = nan_bad_profile_sections(acm_L1(ii)); end
[pr_min, binsize, pr_max] = determine_binning_parameters(acm_L1_Nand, 'pressure');
%.. set L2 sensor_field_indices:
%.. .. (a) to determine which fields to bin
%.. .. (b) for making MPacm_L2 data product
aqd = aqd_set_sensor_field_indices(acm_L1_Nand, 'L2');
...
%..   use the median value found in each acm profile. absent acm 
%..   profiles will have entries of nan.
profile_datenumbers = cellfun(@nanmedian,{acm_L2(prof2proc).time});
...
MP_binned_acm = write_field_arrays_to_new_structure(acm_L2, 'binned_acm_');
...
if createMatfile
    ...
    MP_acm_L1_Nand = write_field_arrays_to_new_structure(acm_L1_Nand);
    MP_beam_mapping.beam_mapping = {acm_L1.beam_mapping};
    ...
    MP_acm_L0_data = write_field_arrays_to_new_structure(acm_L0);
    MP_beam_mapping.beam_mapping = {acm_L0.beam_mapping};
