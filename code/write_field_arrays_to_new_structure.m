function struct_out = write_field_arrays_to_new_structure(array_of_structures, prefix)
%=========================================================================
% DESCRIPTION
%   Horizontally concatenates column vectors of (different) lengths found in
%   designated fields of a structure array by padding with NaNs when necessary
%   and writes these out to a new structure sss using fieldnames formed by
%   prepending a prefix to the original fieldnames.
%
% USAGE:  sss = write_field_arrays_to_new_structure(structArray, prefix) 
%
%   INPUT
...
%.. get the sensor field indices without assuming that the
%.. first element of the structure array has it.
%.. there shouldn't be any nans, but check anyway.
tf_isnan = cell2mat(cellfun(@(x)any(isnan(x)), ...
    {array_of_structures.sensor_field_indices}, 'UniformOutput', false));
tf_isempty = cellfun('isempty', ...
    {array_of_structures.sensor_field_indices});
xtruct = array_of_structures(~(tf_isnan | tf_isempty));
...
for ii=1:nrowC
%     [ii size(cell2mat(C(ii, :)))]
%     [npts_max ndim2(ii) ncolC]
